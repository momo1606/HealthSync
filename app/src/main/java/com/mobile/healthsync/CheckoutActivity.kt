...
class CheckoutActivity : BaseActivity() {
    ...
    private var patientId: Int = 251
    private var appointmentId: Int = 663
    private var doctorId: Int = 663
    ...
    private fun redeemPoints() {
        ...
        val redeemablePoints = rewardPoints - (rewardPoints % pointsNeededForDiscount)
    ...
    private fun createPaymentRecordAndLinkToAppointment(appointmentId: Int, patientId: Int, amountPaid: Double) {
        val db = FirebaseFirestore.getInstance()
        val uniquePaymentId = generateUniquePaymentId()
        ...
        db.collection("payments").add(paymentData)
            .addOnSuccessListener { documentReference ->
                ...
                linkPaymentIdToAppointment(uniquePaymentId, appointmentId)
                addRewardPoints(patientId, 20)
            }
            .addOnFailureListener { e ->
    ...
    private fun generateUniquePaymentId(): Int {
        val timestampPart = (System.currentTimeMillis() % 100000).toInt()
        val randomPart = Random().nextInt(900) + 100
